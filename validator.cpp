...
struct company {
	int stocks_number;//number of company's stocks
	int price_of_stock;
	int budget;
	int invstr_num;//number of investors
	char name[MAX_ARRAY_SIZE];//company's name
	char investors[MAX_ARRAY_SIZE][MAX_STR_SIZE];
	int investors_stocks_num[MAX_ARRAY_SIZE];//number of investors' stocks
	char investing_date[MAX_ARRAY_SIZE][MAX_STR_SIZE];//date of each investing
};
struct client {
	char name[MAX_ARRAY_SIZE];//customer's name
	int budget;//customer's main budget
	int invstd_num;//number of companies customer invested in
	char invested[MAX_ARRAY_SIZE][MAX_STR_SIZE];//name of each company
	int invested_stocks_num[MAX_ARRAY_SIZE];//number of companies stocks
	char invested_date[MAX_ARRAY_SIZE][MAX_STR_SIZE];//date of investing
	int remained_budget[MAX_ARRAY_SIZE];//remained budget after each investing
};
...
int create_directories(char path_name[MAX_STR_SIZE])
{
	...
	int vfile_fd = creat(vfpath, O_RDWR | O_APPEND | O_CREAT);
	//chmod(vfpath,S_IRUSR | S_IWUSR);
...
{
	if(argn < 2){
		char invalid_argErr[] = "Valid format is :\n./Server port_number directory_name\n";
		write(STDOUTFD, invalid_argErr, strlen(invalid_argErr));
		return 0;
	}
	...
	else 	write(STDOUTFD, "binding successful\n", sizeof("binding successful\n"));
	//listenning
